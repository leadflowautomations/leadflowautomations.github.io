...
(() => {
  ...
  function render() {
    ...
    if (state.step < steps.length) {
      ...
      c.querySelectorAll('[data-option]').forEach(b => b.addEventListener('click', () => { state.data[s.key] = b.dataset.option; state.step++; render(); })); return;
    }
  ...
  async function submitFromForm() {
    ...
    const consent = c.querySelector('#lfqConsent').checked, businessName = c.querySelector('#lfqBusinessName').value.trim(), name = c.querySelector('#lfqName').value.trim(), email = c.querySelector('#lfqEmail').value.trim(), phone = c.querySelector('#lfqPhone').value.trim();
    ...
    state.consent = true; state.data = { ...state.data, businessName, name, email, phone, consent: true, consentTimestamp: new Date().toISOString(), conversationId: getConversationId() };
