...
(() => {
  ...
  function injectStyles() {
    ...
    style.textContent = `.lfq-launch{border:1px solid #5b8cff88!important;background:linear-gradient(135deg,#5b8cff22,#7c5cff22)!important;color:#dce6ff!important}.lfq-overlay{position:fixed;inset:0;z-index:9999;display:grid;place-items:center;padding:18px;background:#02050bcc;backdrop-filter:blur(8px)}.lfq-modal{width:min(560px,100%);max-height:90vh;overflow:auto;background:linear-gradient(145deg,#111b2b,#0b121f);border:1px solid #33445f;border-radius:22px;box-shadow:0 30px 100px #000b;padding:22px;color:#f5f7fb}.lfq-modal-head{display:flex;align-items:center;justify-content:space-between;gap:12px;margin-bottom:8px}.lfq-modal-head b{font-size:1rem}.lfq-close{border:1px solid #33445f;background:#ffffff08;color:#aebbd0;border-radius:10px;width:36px;height:36px;cursor:pointer;font-size:18px}.lfq-progress{color:#91adff;text-transform:uppercase;font-size:.68rem;font-weight:800;letter-spacing:.08em;margin-bottom:8px}.lfq h3{margin:0 0 8px;font-size:1.15rem}.lfq p{color:#9aa7ba;font-size:.84rem;margin:8px 0 16px}.lfq-options{display:grid;gap:9px}.lfq-options button,.lfq #lfqSubmit,.lfq-email-fallback,.lfq-close-result{border:1px solid #344563;background:#ffffff08;color:#eaf0ff;border-radius:12px;padding:12px;text-align:left;cursor:pointer;font-weight:700}.lfq-options button:hover{border-color:#5b8cff;background:#5b8cff18}.lfq-fields{display:grid;gap:9px}.lfq-fields input{width:100%;box-sizing:border-box;background:#0c1524;border:1px solid #2c3b53;border-radius:10px;padding:12px;color:#fff;outline:none}.lfq-fields input:focus{border-color:#5b8cff}.lfq-consent{display:flex;gap:9px;align-items:flex-start;margin:14px 0;color:#9aa7ba;font-size:.75rem;line-height:1.4}.lfq-consent input{margin-top:3px}.lfq #lfqSubmit{width:100%;text-align:center;background:linear-gradient(135deg,#5b8cff,#7c5cff);border:0;margin-top:4px}.lfq #lfqSubmit:disabled{opacity:.55;cursor:wait}.lfq-email-fallback,.lfq-close-result{display:block;width:100%;box-sizing:border-box;text-align:center;background:linear-gradient(135deg,#5b8cff,#7c5cff);border:0;text-decoration:none;margin-top:14px}.lfq-close-result{color:#fff}.lfq-note{font-size:.7rem!important;color:#708097!important;text-align:center}.lfq-status{color:#ffb4b4!important;min-height:20px;margin-bottom:0!important}.lfq-success h3{color:#c8ffd8}.lf-footer{padding:34px 0 24px}.lf-footer-grid{display:grid;grid-template-columns:1.2fr 1fr 1fr;gap:30px;align-items:start}.lf-footer-brand{font-weight:800;font-size:.95rem;color:#eaf0ff;margin-bottom:7px}.lf-footer-copy{color:#718096;font-size:.75rem;max-width:300px}.lf-footer-title{color:#cdd7e8;font-size:.72rem;font-weight:800;text-transform:uppercase;letter-spacing:.08em;margin-bottom:10px}.lf-footer-links{display:grid;gap:7px}.lf-footer-links a{color:#8d9bb0;font-size:.75rem}.lf-footer-links a:hover{color:#fff}.lf-footer-contact{display:grid;gap:8px}.lf-footer-email{color:#9eb5ff!important;font-size:.75rem}.lf-footer-cta{display:inline-block;background:linear-gradient(135deg,#5b8cff,#7c5cff);color:#fff!important;padding:9px 12px;border-radius:10px;font-weight:750;font-size:.72rem;width:max-content}.lf-footer-bottom{border-top:1px solid #263247;margin-top:25px;padding-top:16px;display:flex;justify-content:space-between;gap:15px;color:#64748b;font-size:.68rem}@media(max-width:650px){.lf-footer-grid{grid-template-columns:1fr 1fr}.lf-footer-brand-col{grid-column:1/-1}.lf-footer-bottom{flex-direction:column}}`;
    document.head.appendChild(style);
  }

  function removeTalkToDavidChip() {
    document.querySelectorAll('.chips .chip').forEach(chip => {
      const label = chip.textContent.trim().toLowerCase();
      if (label === 'talk to david' || label.includes('talk to david')) chip.remove();
    });
  }

  function enhanceFooter() {
    const footer = document.querySelector('.footer');
    if (!footer || footer.dataset.lfEnhanced === 'true') return;
    footer.dataset.lfEnhanced = 'true';
    footer.innerHTML = `<div class="wrap lf-footer"><div class="lf-footer-grid"><div class="lf-footer-brand-col"><div class="lf-footer-brand">Lead Flow Automation</div><div class="lf-footer-copy">AI-powered lead systems for small businesses — built to capture, qualify and follow up with better customer conversations.</div></div><div><div class="lf-footer-title">Quick Links</div><div class="lf-footer-links"><a href="#services">Services</a><a href="#process">Process</a><a href="#pricing">Pricing</a><a href="#demo">Assistant</a><a href="#faq">FAQ</a></div></div><div><div class="lf-footer-title">Contact</div><div class="lf-footer-contact"><a class="lf-footer-email" href="mailto:${FALLBACK_EMAIL}">${FALLBACK_EMAIL}</a><a class="lf-footer-cta" href="https://calendly.com/leadflowautomations-dav/free-15-minute-consultation" target="_blank" rel="noopener">Schedule a 15-Minute Consultation →</a></div></div></div><div class="lf-footer-bottom"><span>© 2026 Lead Flow Automation. All rights reserved.</span><span>AI lead systems for small businesses.</span></div></div>`;
  }
  ...
  function loadLiveChat() { if (document.getElementById('leadflow-live-chat')) return; const script = document.createElement('script'); script.id = 'leadflow-live-chat'; script.src = 'leadflow-chat.js?v=20260831-6'; script.async = true; document.head.appendChild(script); }
  ...
  function initLauncher() {
    ...
    if (!document.documentElement.dataset.lfqChipObserver) {
      document.documentElement.dataset.lfqChipObserver = 'true';
      const observer = new MutationObserver(() => removeTalkToDavidChip());
