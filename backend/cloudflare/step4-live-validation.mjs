...
console.log(JSON.stringify({
  ...
  statuses: step3.prospects.map(p => ({ name: p.name, research: p.research?.status, website: p.website }))
}, null, 2));

console.log('Step 4 validation: scoring the exact Step 3 output...');
...
if (step4.version !== '2026-09-06.1') throw new Error(`Unexpected Step 4 version: ${step4.version}`);
...
console.log(JSON.stringify({
  ...
  results: step4.prospects.map(p => ({
    name: p.name,
    score: p.score.score,
    opportunity: p.score.opportunity,
    priority: p.score.priority,
    confidence: p.score.confidence,
    topGaps: p.score.gaps.slice(0, 3).map(g => g.label)
  }))
}, null, 2));
