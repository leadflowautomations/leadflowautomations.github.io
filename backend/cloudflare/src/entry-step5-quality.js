...
function normalizePhoton(features,country){
  ...
  for(const f of features||[]){
    ...
    out.push({name,website,phone,email,socials:[],address:photonAddress(p,country),latitude:lat,longitude:lon,source:'Photon/OpenStreetMap',evidence:[website?'Public website listed':'No public website listed',phone?'Public phone listed':'No public phone listed',email?'Public email listed':'No public email listed','Discovery recovered from Photon after primary Overpass providers failed.']});
  }
  return out;
}
...
async function photonFallback(location,country,industry){
  ...
  if(!geo.ok)throw new Error(`Fallback location lookup failed (HTTP ${geo.status}).`);
  const places=await geo.json();
  if(!places?.[0])throw new Error(`Fallback could not locate ${location}, ${country}.`);
  ...
  if(!prospects.length)throw new Error('Photon fallback returned no matching businesses.');
  return {prospects,location,country,industry,center:{lat,lon},discoveryRadiusKm:20,discoveredCount:prospects.length,returnedCount:prospects.length,discoveryLimit:1000,source:'Photon/OpenStreetMap',discoveryProvider:'Photon fallback',fallback:true,discoveryNote:'Primary Overpass discovery providers were unavailable, so Lead Flow used Photon/OpenStreetMap category search across the same 20 km market radius.'};
}
...
export default {async fetch(request,env,ctx){
  ...
  if(request.method==='GET'&&url.pathname==='/api/prospect-search'){
    ...
    try{
      ...
      return json({ok:true,stage:'discover',version:'2026-09-06.9',...d},200,origin);
    }catch(fallbackError){
      if(primary.status>=400){try{const data=await primary.clone().json();return json({...data,error:`${data?.error||'Primary discovery failed.'} Photon fallback also failed: ${fallbackError?.message||'fallback failed'}`},502,origin)}catch{}}
      return json({ok:false,error:fallbackError?.message||'Discovery failed.'},502,origin);
    }
  }
