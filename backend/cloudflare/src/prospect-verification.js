...
const categoryMatch=(p,industry)=>{
  ...
  const match=words.some(w=>hay.includes(norm(w)));
  if(match)return {match:true,confidence:'high',reason:`Business record contains ${industry}-relevant category evidence.`};
...
async function reachable(url){
  ...
  try{
    let r=await fetch(target,{method:'HEAD',redirect:'follow',headers:{accept:'text/html,application/xhtml+xml','user-agent':'LeadFlowAutomation/1.0'},signal:controller.signal});
    if(!r.ok && (r.status===405||r.status===403||r.status===400)){
      r=await fetch(target,{method:'GET',redirect:'follow',headers:{accept:'text/html,application/xhtml+xml','range':'bytes=0-2048','user-agent':'LeadFlowAutomation/1.0'},signal:controller.signal});
    }
    return {checked:true,reachable:r.ok,status:r.status,url:r.url||target,reason:r.ok?'Website responded successfully':`Website returned HTTP ${r.status}.`};
  }catch(error){return {checked:true,reachable:false,url:target,reason:error?.name==='AbortError'?'Website check timed out':'Website could not be reached.'};}
...
export async function verifyProspects(prospects,industry){
  const list=Array.isArray(prospects)?prospects.slice(0,100):[];
  ...
  return results.map((p,index)=>{
    ...
    for(const [key,prior] of seen){
      const sameName=nameKey&&key.startsWith(nameKey+'|');
      const sameAddress=addressKey&&prior.addressKey===addressKey;
      const near=Number.isFinite(lat)&&Number.isFinite(lon)&&Number.isFinite(prior.lat)&&Number.isFinite(prior.lon)&&Math.abs(lat-prior.lat)<0.0015&&Math.abs(lon-prior.lon)<0.0015&&nameKey===prior.nameKey;
      if(sameName&&(sameAddress||near)){duplicateOf=prior.index;break;}
    }
    if(nameKey)seen.set(`${nameKey}|${addressKey}`,{index:index+1,nameKey,addressKey,lat,lon});
    const v=p.__verification;delete p.__verification;
    const unique=!duplicateOf;const verified=Boolean(v.active&&v.category.match&&unique);
    return {...p,verification:{verified,active:v.active,activeConfidence:v.activeConfidence,categoryMatch:v.category.match,categoryConfidence:v.category.confidence,unique,duplicateOf,website:{checked:v.website.checked,reachable:v.website.reachable,status:v.website.status||null,reason:v.website.reason||'',url:v.website.url||p.website||''},evidence:[v.active?(v.website.reachable?'Website responded successfully':'Public phone/email signal found'):'No active public signal found',v.category.reason,unique?'No duplicate match found':`Duplicate of result #${duplicateOf}`]}};
  });
}
