...
async function researchAndScore(prospects,{location='',industry=''}={}){
...
export default {async fetch(request,env,ctx){
  ...
  if(url.pathname==='/api/prospect-score'){
    ...
    try{const body=await request.json(),prospects=Array.isArray(body?.prospects)?body.prospects:[];const location=String(body?.location||'').trim(),industry=String(body?.industry||'').trim().toLowerCase();if(!prospects.length)return json({ok:false,error:'prospects are required.'},400,origin);if(prospects.length>100)return json({ok:false,error:'A maximum of 100 prospects can be scored at once.'},400,origin);const output=await researchAndScore(prospects,{location,industry});return json({ok:true,stage:'score',version:'2026-09-06.1',location,industry,summary:{scored:output.length,veryHigh:output.filter(p=>p.score.opportunity==='Very high').length,high:output.filter(p=>p.score.opportunity==='High').length,moderate:output.filter(p=>p.score.opportunity==='Moderate').length,low:output.filter(p=>p.score.opportunity==='Low').length},prospects:output},200,origin)}catch(error){return json({ok:false,error:error?.message||'Scoring failed.'},500,origin)}
  }
  if(url.pathname==='/api/prospect-signals'){
    if(request.method==='OPTIONS')return new Response(null,{status:204,headers:cors(origin)});
    if(request.method!=='POST')return json({ok:false,error:'Method not allowed'},405,origin);
    try{const body=await request.json(),prospects=Array.isArray(body?.prospects)?body.prospects:[];const location=String(body?.location||'').trim(),industry=String(body?.industry||'').trim().toLowerCase();if(!prospects.length)return json({ok:false,error:'prospects are required.'},400,origin);if(prospects.length>100)return json({ok:false,error:'A maximum of 100 prospects can be researched at once.'},400,origin);const researched=await researchProspects(prospects,{location,industry}),output=researched.map(p=>({...p,signals:buildSignals(p),researchedAt:new Date().toISOString()})),summary={researched:output.length,inspected:output.filter(p=>p.research?.status==='inspected').length,websiteFound:output.filter(p=>p.research?.status==='website-found').length,publicRecordOnly:output.filter(p=>p.research?.status==='public-record-only').length};return json({ok:true,stage:'research-signals',version:'2026-09-06.2',location,industry,summary,prospects:output},200,origin)}catch(error){return json({ok:false,error:error?.message||'Signal research failed.'},500,origin)}
  }
