...
try:
    ...
    async def _photon_search(client, query, lat, lon, delta):
        await asyncio.sleep(0.25); half = delta / 2
        r = await client.get("https://photon.komoot.io/api/", params={"q": query, "limit": 40, "bbox": f"{lon-half},{lat-half},{lon+half},{lat+half}"}, headers={"User-Agent": _lf.UA, "Referer": "https://leadflowautomations.github.io/"})
        r.raise_for_status(); converted = []
        for feature in r.json().get("features", []):
            p = feature.get("properties") or {}; c = (feature.get("geometry") or {}).get("coordinates") or []
            name = (p.get("name") or p.get("street") or "").strip()
            if not name: continue
            raw_type = str(p.get("osm_type") or "")
            osm_type = {"N":"node", "W":"way", "R":"relation", "node":"node", "way":"way", "relation":"relation"}.get(raw_type, raw_type)
            converted.append({"osm_type": osm_type, "osm_id": p.get("osm_id"), "name": name, "display_name": ", ".join(str(p.get(k)) for k in ("name","street","city","state","postcode","country") if p.get(k)), "lat": str(c[1]) if len(c)>=2 else None, "lon": str(c[0]) if len(c)>=2 else None, "address": {"house_number":p.get("housenumber"),"road":p.get("street"),"city":p.get("city") or p.get("locality"),"state":p.get("state"),"postcode":p.get("postcode")}, "extratags": {}})
        return converted

    _original_geocode = _lf.geocode; _original_search = _lf.nominatim_search
    ...
    _lf.geocode = _resilient_geocode; _lf.nominatim_search = _resilient_search
